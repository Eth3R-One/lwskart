...
const Form = ({ open, setOpen, user, address }) => {
  ...
  const fullAddress = address?.billingAddress?.split("+ ");
  ...
  const handleSubmit = async (event) => {
    ...
    const isChanged = address?.billingAddress != updatedLocation;
    if (isChanged) {
      try {
...
const BillingAddressForm = ({ dictionary, user, address }) => {
          ...
          <div className="flex flex-row gap-3 border p-2 rounded-lg items-center py-2">
            ...
            {address ? (
              <p className="text-gray-800">
                {address?.billingAddress?.split("+")?.join(", ")}
              </p>
            ) : (
